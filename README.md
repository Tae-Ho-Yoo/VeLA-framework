...
This project implements a real-time emotion-aware human-robot interaction system for robotic handover.

The system integrates:
- Large Language Model (Qwen) for interaction reasoning
- Vision-Language-Action model (OpenVLA) for motion generation
- Personalized comfort-zone modeling for adaptive speed control

Key 특징:
- Trajectory는 유지하고 speed만 modulation
- Emotion-driven continuous speed adaptation
- User-specific comfort zone learning
- Closed-loop interaction 가능 구조

## Pipeline

instruction + emotion_history + user_id
    → personalized comfort zone lookup
    → base speed selection
    → Qwen reasoning (task + pace + comfort)
    → OpenVLA (action generation)
    → execution speed modulation
    → user profile update

## To run server-based framework
1. start both servers following README in vela-survey-web
2. pip install -r requirements.txt
3. conda env create -f environment.yml
4. conda activate <env_name>
5. run python .\vla-scripts\deploy_discrete.py

## To run fixed framework(for cross-validation where you need fixed task/speed)
1. pip install -r requirements.txt
2. conda env create -f environment.yml
3. conda activate <env_name>
4. python deploy_test.py --mode fixed_eval --eval_image_path <image>
