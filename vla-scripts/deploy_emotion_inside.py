...
def get_reasoner_prompt(instruction: str, emotion_history: list[str]) -> str:
...
Your job:
1. Infer the user's emotional trend from the history.
...
- When the user appears more comfortable, the current action is often appropriate. Prefer continuing the current motion unless a clear transition is needed.
- When the user appears uncomfortable, prefer safer, slower, or more careful motion.
- Do not jump to distant future actions. Focus only on the immediate next step.
- Treat emotion as soft guidance, not strict rules.

Examples:

Instruction: give me the cup
Emotion History: neutral, neutral, positive
Reasoning: The user appears more comfortable, so the current motion should continue naturally.
Task: continue moving toward the cup

Instruction: give me the cup
Emotion History: neutral, neutral, neutral
Reasoning: The user shows no strong emotional change, so continue normally.
...
def parse_reasoning_task(generated_text: str):
    ...
    parse_success = bool(reasoning or task)
...
class OpenVLAServer:
    ...
    def refine_instruction(self, instruction: str, emotion_history: list[str]) -> Dict[str, str]:
        prompt = get_reasoner_prompt(instruction, emotion_history)
    ...
    def predict_action(self, payload: Dict[str, Any]) -> JSONResponse:
        try:
           ...
           # Qwen reasoning module
            reasoner_result = self.refine_instruction(instruction, emotion_history)
